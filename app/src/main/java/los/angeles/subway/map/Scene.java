...
class Scene {
	...
	class Viewport {
		...
		final Rect origin = new Rect(0,0,0,0);  // fine for 1024 height

		void setOrigin(int x, int y){
			synchronized(this){ // width height is for the pic!
				int w = origin.width();
				int h = origin.height();
	...
	class Cache {
		 ...
		 */
		int percent = 8; // Above 25 and we get OOMs
